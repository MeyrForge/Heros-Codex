...
class GenerateFeaturesUseCaseTest {
  ...
  @Test
  fun `should generate non-empty features list for given character class`() {
  ...
  @Test
  fun `should generate correct number of features for each class`() {
    val expectedFighterFeatures = Feature.getFeaturesForClass(testFighter, 1)
    val expectedWizardFeatures = Feature.getFeaturesForClass(testWizard, 1)

    every {
      characterRepository.getFeaturesForClass(
        testFighter,
        1
      )
    } returns expectedFighterFeatures
    every { characterRepository.getFeaturesForClass(testWizard, 1) } returns expectedWizardFeatures

    val fighterFeatures = useCase.generate(testFighter)
    val wizardFeatures = useCase.generate(testWizard)

    assertEquals("Fighter should have 2 features at level 1", 2, fighterFeatures.size)
    assertEquals("Wizard should have 2 features at level 1", 2, wizardFeatures.size)
  }

  @Test
  fun `Fighter should have specific class features`() {
    val expectedFeatures = Feature.getFeaturesForClass(testFighter, 1)
    every { characterRepository.getFeaturesForClass(testFighter, 1) } returns expectedFeatures
  ...
  @Test
  fun `Wizard should have specific class features`() {
  ...
  @Test
  fun `should generate valid features for all available classes`() {
    CharacterClass.AVAILABLE_CLASSES.forEach { characterClass ->
      ...
      assertTrue("Features for ${characterClass.name} should not be empty", features.isNotEmpty())
      assertTrue(
  ...
  fun `generated features should have unique ids`() {
    ...
    assertEquals("All feature IDs should be unique", featureIds.size, uniqueIds.size)
  }

  @Test
  fun `should call repository when generating features`() {
    val expectedFeatures = Feature.getFeaturesForClass(testFighter, 1)
    every { characterRepository.getFeaturesForClass(testFighter, 1) } returns expectedFeatures

    useCase.generate(testFighter)
